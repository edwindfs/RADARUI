    ...
    def redraw(self, active_targets, selected_tid, kinematics):
        ...
        for tid, details in kinematics.items():
            ...
            if isinstance(tid, int):
                ...
                ip, track_id = tid
                import echoshield
                import rada_connection as rada
                if ip == rada.RADA_IP:
...
class OperatorApp:
    ...
    def _authorize_once(self):
        if self.c2.selected_target_id:
            ...
            success = self.c2.send_coordinate_once(tid)
            ...
            if success:
                ...
                self.status_var.set(f"FAILED TO SEND ONE-SHOT {display_tid}.")
    ...
    def refresh(self):
        ...
        if self.c2.selected_target_id:
            is_valid = False
            with self.c2.lock:
                if isinstance(self.c2.selected_target_id, int):
                    is_valid = self.c2.selected_target_id in self.c2.fused_targets
                else:
                    is_valid = self.c2.selected_target_id in self.c2.active_targets
    ...
    def on_close(self):
        ...
        if hasattr(self, 'rada_manager'):
            self.rada_manager.stop()
        self.root.destroy()
